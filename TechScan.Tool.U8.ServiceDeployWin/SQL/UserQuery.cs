...
using TechScan.Tool.U8.ServiceDeployWin.SQL.Diff;
...
namespace TechScan.Tool.U8.ServiceDeployWin.SQL
{
    ...
    {
        private readonly DbServerInfo _server = null;
        ...
        public UserQuery(string query, DbServerInfo server)
            : this()
        {
                ...
                if (results != null)
                {
                    this.Invoke(new Action(() =>
                   {
                       ...
                       if (tpData.Controls.Count > 0)
                       {
                           ...
                           tpData.Controls.Cast<System.Windows.Forms.Control>().ForEach(c => c.Height = height);
                       }
                       time.Stop();
        ...
        public void Execute()
        {
            if (!_isRunning)
            {
                ...
                {
                    MessagesHelper.ShowMessage("Please input sql to execute.", MessageBoxIcon.Warning);
                }
            }
            else
            {
        ...
        private void SaveScript()
        {
            ...
            var newFile = frmSQLDeploy.Instance.SaveScript(_fileName, rtbSQL.Text);
            if (!string.IsNullOrEmpty(newFile))
            {
                _fileName = newFile;
                frmSQLDeploy.Instance.AddRecentObject(_server, _server.Database, _fileName, RecentObjectTypes.FilePath, string.Empty);
            ...
            using (var dlg = new OpenFileDialog())
            {
                if (dlg.ShowDialog(this.ParentForm) == DialogResult.OK)
                {
                    ...
                    if (!string.IsNullOrEmpty(rtbSQL.ActiveTextAreaControl.TextArea.SelectionManager.SelectedText))
                        FormOpUtils.SelectText(rtbSQL, text);
                    else
                        rtbSQL.Text = text;
                    frmSQLDeploy.Instance.AddRecentObject(_server, _server.Database, _fileName, RecentObjectTypes.FilePath, string.Empty);
                }
        ...
        {
            var result = FormOpUtils.GetDragDropContent(e);
        ...
        {
            FormOpUtils.HandleContentDragEnter(e);
        }

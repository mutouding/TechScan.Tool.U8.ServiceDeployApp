...
namespace TechScan.Tool.U8.ServiceDeployWin.SQL.Diff
{
    ...
    public class DiffResults : Form
    {
        ...
        public DiffResults()
        {
            ...
            this.Icon = Icon.FromHandle(TechScan.Tool.U8.ServiceDeployWin.Properties.Resources.History2.GetHicon());

            FormOpUtils.SetTextBoxStyle(_rtbSourceScript);
            FormOpUtils.SetTextBoxStyle(_rtbDestinationScript);

            //_rtbSourceScript.CurrentLineColor = Color.Blue;
            //_rtbDestinationScript.CurrentLineColor = Color.Blue;
        ...
        public DiffResults(string sourceScript)
            : this()
        {
            _currentVersion = FormOpUtils.EmptyIndex;
            _previousVersion = FormOpUtils.EmptyIndex;
            ...
            : this()
        {
            ...
            if (_previousVersion == FormOpUtils.EmptyIndex)
                _cboSources.Visible = false;
            else
            {
                //////////_cboSources.DataSource = frmSQLDeploy.Instance.GetObjectVersions();
                //////////_cboSources.SelectedValue = _previousVersion;
            }

            _currentVersion = currentVersion;
            if (_currentVersion == FormOpUtils.EmptyIndex)
                _cboDestinations.Visible = false;
            else
            {
                ////////_cboDestinations.DataSource = frmSQLDeploy.Instance.GetObjectVersions();
        ...
        {
            var sourceScript = previousVersion != FormOpUtils.EmptyIndex ? GetVersionScript(previousVersion) : DefaultCompare;
            var destinationScript = currentVersion != FormOpUtils.EmptyIndex ? GetVersionScript(currentVersion) : string.Empty;
        ...
        {
            if (version == 0) return "";
            //////return frmSQLDeploy.Instance.GetObjectScriptTextEx();
            else
                //////return frmSQLDeploy.Instance.GetObjectScriptVersionText(version);
                return "";
        }
        ...
        {
            if (_isLoaded)
                TextDiff(_previousVersion != FormOpUtils.EmptyIndex && _cboSources.SelectedValue != null ? Convert.ToInt32(_cboSources.SelectedValue) : FormOpUtils.EmptyIndex
                    , _currentVersion != FormOpUtils.EmptyIndex && _cboDestinations.SelectedValue != null ? Convert.ToInt32(_cboDestinations.SelectedValue) : FormOpUtils.EmptyIndex);
        }
